...
void load_tiles(struct map * map, unsigned char * tile_bmp, unsigned short nb_tiles, unsigned short transp_color)
{
	...
	{
		map->tab_tiles[cpt].img[0] = tile_bmp + (cpt*map->size_tile_x*map->size_tile_y) ;
		...
		if(transp_color)
		{
			...
			do	// bound up
			{
				...
				do
				{
					...
					color = *((unsigned char *)map->tab_tiles[cpt].img[0] + cpt_x*map->size_tile_y + (map->size_tile_y-cpt_y)) ;
...
void load_animated_tile(struct map * map, unsigned char * tile_bmp, unsigned short nb_tiles, unsigned short id_tile, unsigned char type_anim, unsigned short time_anim, unsigned char frame_init)
{
	...
	for(cpt=0;cpt<nb_tiles;cpt++)
	{
		map->tab_tiles[id_tile].img[cpt] = tile_bmp + (cpt*map->size_tile_x*map->size_tile_y) ;
		...
		if(map->transp_color)
		{
			...
			do	// bound up
			{
				...
				do
				{
					...
					color = *((unsigned char *)map->tab_tiles[id_tile].img[cpt] + cpt_x*map->size_tile_y + (map->size_tile_y-cpt_y)) ;
...
void afficher_tile_cut(struct map * map, short x, short y, unsigned char* sprite,unsigned short id)
{
	...
	if(map->tab_tiles[id].transp[map->tab_tiles[id].anim_id] != 0xFF)
	{
		...
		{
			GpTransBlt(NULL,&gpDraw[nflip], aff_x, aff_y, taille_x, taille_y, sprite, debut_x, debut_y,map->size_tile_x,map->size_tile_y,map->transp_color) ;
		}
		else
		{
			GpBitBlt(NULL,&gpDraw[nflip], aff_x, aff_y, taille_x, taille_y, sprite, debut_x, debut_y,map->size_tile_x,map->size_tile_y) ;
		}
	}
}
...
void draw_tile_xy(struct map * map, unsigned short id,unsigned short x,unsigned short y)
{
	...
	if(map->tab_tiles[id].transp[map->tab_tiles[id].anim_id] != 0xFF)
	{
		...
		if((map->tab_tiles[id].transp[map->tab_tiles[id].anim_id])&&(tile_draw_method))
		{
			GpTransBlt(NULL,&gpDraw[nflip], aff_x, aff_y, taille_x, taille_y, map->tab_tiles[id].img[map->tab_tiles[id].anim_id], debut_x, debut_y,map->size_tile_x,map->size_tile_y,map->transp_color) ;
		}
		else
		{
			GpBitBlt(NULL,&gpDraw[nflip], aff_x, aff_y, taille_x, taille_y, map->tab_tiles[id].img[map->tab_tiles[id].anim_id], debut_x, debut_y,map->size_tile_x,map->size_tile_y) ;
		}
	}
...
/* affiche une map sur l'ecran entier, animée grace a la taille du tile*/

void draw_map_animated(struct map * map, int size_x, int size_y)
{
	...
	debut_case_aff_x = map->scrool_x/size_x ;
	decalage_case_x = ((debut_case_aff_x+1)*map->size_tile_x) - map->scrool_x ;

	debut_case_aff_y = map->scrool_y/size_y ;
...
void slow_auto_scrool(struct map * map, short pos_x, short pos_y, short size_x, short size_y, short limit_up, short limit_down, short limit_right, short limit_left, short time, short powa)
{
	static long last=0 ;

	if(GpTickCountGet() - last > time)
	{
		last = GpTickCountGet() ;
		// test gauche
		if((pos_x - map->scrool_x) < limit_left)
...
void ultimate_auto_scrool(struct map * map, short pos_x, short pos_y, short size_x, short size_y, short limit_up, short limit_down, short limit_right, short limit_left, short blimit_up, short blimit_down, short blimit_right, short blimit_left,short time, short powa)
{
	static long last=0 ;

	if((pos_x - map->scrool_x) < blimit_left)	map->scrool_x -= blimit_left - (pos_x - map->scrool_x) ;
	else	if(((pos_x+size_x) - map->scrool_x) > (320-blimit_right))	map->scrool_x += ((pos_x+size_x) - map->scrool_x) - (320-blimit_right) ;

	if((pos_y - map->scrool_y) < blimit_up)	map->scrool_y -= blimit_up - (pos_y - map->scrool_y) ;
	else	if(((pos_y+size_y) - map->scrool_y) > (240-blimit_down))	map->scrool_y += ((pos_y+size_y) - map->scrool_y) - (240-blimit_down) ;
	...
	if(GpTickCountGet() - last > time)
	{
		...
		if((pos_y - map->scrool_y) < limit_up)
		{
			if((pos_y - map->scrool_y) < limit_up>>1) powa++ ;
			map->scrool_y -=powa ;
		}
		else
		{
...
void slow_folow_gfx(struct map * map, struct gfx * gfx, short limit_up, short limit_down, short limit_right, short limit_left, short time,short powa)
{
	slow_auto_scrool(map,gfx->pos_x,gfx->pos_y,gfx->size_x,gfx->size_y,limit_up,limit_down,limit_right,limit_left,time,powa) ;
}

void ultimate_folow_gfx(struct map * map, struct gfx * gfx, short limit_up, short limit_down, short limit_right, short limit_left, short blimit_up, short blimit_down, short blimit_right, short blimit_left, short time,short powa)
{
	ultimate_auto_scrool(map,gfx->pos_x,gfx->pos_y,gfx->size_x,gfx->size_y,limit_up,limit_down,limit_right,limit_left,blimit_up,blimit_down,blimit_right,blimit_left,time,powa) ;
}
