...
/*	if(strlen(title_text)>1) // if too many title char to save the pos
	{
	...
	if(strlen(title_text) > max_size) max_size = strlen(title_text) ;
	...
	//if(strlen(title_text)>1) pos[(title_text[0]+title_text[1])/2] = item_selected ; // save_pos
