...
//-------------------------------------------------------------------------------------
void taille_pcx(char * fichier, ulong * largeur, ulong * hauteur)
{
    ulong	p_read_count;
    F_HANDLE flot;
...
#define getPcxSy getPcxHeight
int getPcxHeight(char * fileName)
{
	F_HANDLE file ;
	ushort size_y ;
	
	GpFileOpen(fileName, OPEN_R, &file);
	GpFileSeek(file, FROM_BEGIN, 10, 0);
	GpFileRead(file,&size_y,2,NULL);
	GpFileClose(file) ;
	return size_y+1 ;
}
...
void rotate_90(uchar * buffer_src, uchar * buffer_dest, ulong largeur, ulong hauteur)
{
	ulong cptx,cpty ;
	uchar color ;
...
void rotate_90_and_draw(uchar * buffer_src, uchar * buffer_dest, ulong largeur, ulong hauteur, struct font * font, ulong posx, ulong posy)
{
	ulong cptx,cpty ;
	uchar color ;
...
uchar * load_pcx_for_pcx_viewer(char * pcx_path,uchar ** buf_unrotate, struct font * font)
{
	...
	sprintf(buf,"%s, %i*%i, %i B",pcx_path+8,largeur,hauteur,taille_fichier) ;
...
{
	ulong largeur, hauteur, nb_char, tr_color ;
	uchar * img_font ;
	char buffer[150] ;
	...
	{
		sprintf(buffer,"unable to load font\n%s",pcx_path) ;
		MsgBox(buffer) ;
	}
}

void load_unsquare_font_from_pcx(struct font * font, char * pcx_path, ulong size_letter_x)
{
	ulong largeur, hauteur, nb_char, tr_color ;
	uchar * img_font ;
