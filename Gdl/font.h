...
void load_font(struct font * font, unsigned char * img_font, unsigned char sx, unsigned char sy, unsigned short tr_color, unsigned char nb_char)
{
	...
	{
		font->img[cpt+32] = img_font + (cpt*sx*sy) ;
	} ;
}
...
void load_font_(struct font * font, unsigned char * img_font, unsigned char sx, unsigned char sy, unsigned short tr_color, unsigned char nb_char, unsigned char start)
{
	...
	for(cpt=0;cpt<nb_char;cpt++)
	{
		font->img[cpt+start] = img_font + (cpt*sx*sy) ;
	} ;
}
...
void draw_text(struct font * font,char * text, unsigned short x, unsigned short y)
{
	unsigned short cpt;
	short cpt_txt ;
	unsigned short nb_char = strlen(text) ;
...
void draw_text_rl(struct font * font, char * text, unsigned short x, unsigned short y)
{
	unsigned short cpt;
	short cpt_txt ;
	unsigned char nb_char = strlen(text) ;
...
void draw_text_center(struct font * font, char * text)
{
	draw_text(font,text,((320 - (strlen(text)*font->size_x))>>1),((240 - (font->size_y))>>1)) ;
}

void draw_text_center_x(struct font * font, char * text, unsigned short y)
{
	draw_text(font,text,((320 - (strlen(text)*font->size_x))>>1),y) ;
}

void draw_text_center_x_multiline(struct font * font, char * text, unsigned short y)
{
...
void draw_text_center_y(struct font * font, char * text, unsigned short x)
{
...
{
	draw_text(font,text,((320 - (strlen(text)*font->size_x))>>1)+(powa*((rand()%2)-1)),((240 - (font->size_y))>>1)+(powa*((rand()%2)-1))) ;
}

void draw_text_center_x_vibrate(struct font * font,char * text, unsigned short y, unsigned char powa)
{
	draw_text(font,text,((320 - (strlen(text)*font->size_x))>>1)+(powa*((rand()%2)-1)),y+(powa*((rand()%2)-1))) ;
}
...
void draw_int_center(struct font * font, int value)
{
 	char buffer[6];
 	sprintf(buffer,"%i",value) ;

	draw_text(font,buffer,((320 - (strlen(buffer)*font->size_x))>>1),((240 - (font->size_y))>>1)) ;
}

void draw_int_center_x(struct font * font, int value, unsigned short y)
{
 	char buffer[6];
 	sprintf(buffer,"%i",value) ;

	draw_text(font,buffer, ((320 - (strlen(buffer)*font->size_x))>>1), y);
}

void draw_int_center_y(struct font * font, int value, unsigned short x)
{
 	char buffer[6];
 	sprintf(buffer,"%i",value) ;

	draw_text(font,buffer, x, ((240 - (font->size_y))>>1));
}
