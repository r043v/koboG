...
void read_dir(char * dir, char * extention, char * file_url)
{
	...
	sprintf(buffer,"%s\\%s",dir,file_list[file_selected].name) ;
 	...
 	do {
		...
		sprintf(buf,"%i/%i",file_selected-1,file_number-2) ;
		...
		for(file_map=start;(file_map<file_number)&&(num_draw<limit_draw);file_map++)
		{
			...
			if(!strcmp(extention,(char*)ext))
			{
				if(file_selected==file_map)	sprintf(buf,"* %s *",file_list[file_selected].name) ;
				else				sprintf(buf,"%s",file_list[file_map].name) ;
		...
		if((up_front(159))||(instant_key(144)&&(old_tick+20<GpTickCountGet()))||(instant_key(10)&&(old_tick+90<GpTickCountGet()))||(instant_key(5)&&(old_tick+250<GpTickCountGet())))
		{
				...
				for(cpt=0;cpt<10;cpt++)
				{
					if(file_selected != file_number-1)
					{
						...
						while((!ok)&&(file_selected != file_number))
						{
							...
							if(strcmp(extention,(char*)ext)) file_selected++ ;
							else ok=1 ;
						}
			...
			if(old_file_selected != file_selected)	sprintf(buffer,"%s\\%s",dir,file_list[file_selected].name) ;
		}
